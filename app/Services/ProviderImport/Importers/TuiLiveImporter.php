...
class TuiLiveImporter implements ProviderHttpImporter
{
    ...
    public function import(string $url, SavedHolidaySearch $search, ProviderSource $provider): ProviderImportResult
    {
        ...
        if ($response->status() === 0) {
            throw new ConnectionException('TUI request did not return a response.');
        }
        if (! $response->successful()) {
            throw new \RuntimeException('TUI HTTP '.$response->status().' for '.$url);
        }

        $body = (string) $response->body();
        $candidates = $this->candidatesFromBody($body, $search, $provider, $url);
        if ($candidates === []) {
            throw new \RuntimeException('TUI payload did not contain parsable holiday candidates.');
        }
    ...
    private function candidatesFromBody(string $body, SavedHolidaySearch $search, ProviderSource $provider, string $url): array
    {
        ...
        foreach ($docs as $doc) {
            ...
            $candidates[] = [
                'provider_option_id' => 'tui-'.substr(sha1($url.'|'.$name), 0, 12),
