...
@php
    ...
    $provider = $vm->providerName;
    $score = number_format($vm->overallScore, 1);
...
<article class="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div class="relative h-44 flex-shrink-0 overflow-hidden sm:h-48">
        ...
        @if ($imageUrl !== '')
            <img src="{{ $imageUrl }}" alt="{{ $hotel }}" class="absolute inset-0 h-full w-full object-cover" loading="lazy" />
        ...
        @endif
        <span class="absolute left-3 top-3 inline-flex h-7 w-7 items-center justify-center rounded-full bg-slate-900 text-xs font-bold text-white">{{ $rankLabel }}</span>
        <span class="absolute right-3 top-3 rounded-full bg-white/90 px-2.5 py-0.5 text-xs font-semibold text-slate-800">{{ $provider }}</span>
    </div>
    ...
    <div class="flex flex-1 flex-col p-4 sm:p-5">
        <div class="flex items-start justify-between gap-2">
            ...
            <div class="flex flex-shrink-0 items-end gap-0.5 rounded-lg bg-green-600 px-2.5 py-1.5 text-right text-white sm:px-3 sm:py-2">
                <p class="text-2xl font-bold leading-none sm:text-3xl">{{ $score }}</p>
